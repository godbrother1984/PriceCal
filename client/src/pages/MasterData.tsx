...
// version: 3.2 (Error Display Fix)
// last-modified: 30 สิงหาคม 2568 10:30
...
const FormModal: React.FC<{
  ...
  const renderInput = (column: Column) => {
    ...
    if (column.options) {
      return (
        <select
          ...
          className={`${baseClasses} ${errorClasses} bg-slate-50`}
    ...
    // For foreign key fields (Customer, Customer Group)
...
const MasterDataTable: React.FC<MasterDataTableProps> = ({ 
  ...
  const handleDelete = async (id: string | undefined, itemName?: string) => {
    if (!id) {
      setError('Cannot delete item: Missing ID');
      return;
    }
    ...
    try {
      ...
      setError(err.message || `Failed to delete ${title.slice(0, -1).toLowerCase()}. Please try again.`);
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">{data.length} {data.length === 1 ? 'item' : 'items'} total</p>
        </div>
        <button
      ...
      {data.length === 0 && !error ? (
        <div className="text-center py-12 bg-white rounded-lg border border-slate-200">
          ...
          <h3 className="text-lg font-medium text-slate-900 mb-2">No {title} Found</h3>
...
const CustomerMappingTable: React.FC = () => {
  ...
  const handleDelete = async (id: string | undefined, customerName?: string) => {
    if (!id) {
      setError('Cannot delete item: Missing ID');
      return;
    }
    ...
    <div className="space-y-4">
      ...
      {data.length === 0 && !error ? (
        <div className="text-center py-12 bg-white rounded-lg border border-slate-200">
          ...
          <h3 className="text-lg font-medium text-slate-900 mb-2">No Customer Mappings Found</h3>
          <p className="text-slate-500 mb-4">Start by creating your first customer mapping.</p>
          <button
            onClick={() => {
              setEditingItem(null);
              setIsModalOpen(true);
            ...
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Customer Group</th>
                ...
                {data.map((mapping, index) => (
                  <tr key={mapping.id || index} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-slate-900">{mapping.customerName}</div>
                      <div className="text-sm text-slate-500">{mapping.customerId}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-slate-900">{mapping.customerGroupName || mapping.customerGroupId}</div>
                      <div className="text-sm text-slate-500">{mapping.customerGroupId}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end gap-2">
                        <button
                        ...
                        <button
                          onClick={() => mapping.id && handleDelete(mapping.id, mapping.customerName)}
                          className="text-red-600 hover:text-red-800 font-medium transition-colors"
...
const StandardPrices: React.FC = () => {
  const columns: Column[] = [
    { key: 'rmId', label: 'Raw Material ID', required: true },
